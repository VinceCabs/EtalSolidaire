...
function addTicketsNb(sheet = SpreadsheetApp.getActiveSheet()) {
  ...
  const ticketsColumn = data.map(function (value, index) {
    return [value[ticketsColIdx]];
  });
  ...
  const position = [];
  for (
    let ticketPagePosition = 1;
    ticketPagePosition < TICKETS_PER_PAGE + 1;
    ticketPagePosition++
  ) {
  ...
  r = sheet
    .getRange(2, ticketsColIdx + 1, ticketsColumn.length)
    .setValues(ticketsColumn);
